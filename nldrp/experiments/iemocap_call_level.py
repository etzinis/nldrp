...
def dummy_generate_SVMs_and_LRs():
    svm_params = [('svm', c) for c in [1, 3, 5, 7, 8, 10, 15, 30]]
        ...
        for spkr in temp_dic:
            for _id, el_dic in temp_dic[spkr].items():
                if el_dic['y'] == 'happy' or el_dic['y'] == 'excited':
                    temp_dic[spkr][_id]['y'] = 'happy_excited'
                if final_data_dic[spkr][_id]['y'] == 'happy' or final_data_dic[spkr][_id]['y'] == 'excited':
                    final_data_dic[spkr][_id]['y'] = 'happy_excited'
        freq = {}
        for spkr in final_data_dic:
            for _, raw_dic in final_data_dic[spkr].items():
                freq[raw_dic['y']] = freq.get(raw_dic['y'], 0) + 1
        print freq
        try:
            for spkr in temp_dic:
                for _id, el_dic in temp_dic[spkr].items():
