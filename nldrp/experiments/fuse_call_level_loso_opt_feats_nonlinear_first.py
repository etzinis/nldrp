...
def fusion_loso(list_of_paths):
    ...
    for nl_feat_p, temp_dic in nl_feature_dic.items():
        ...
        try:
            for spkr in temp_dic:
                for id, el_dic in temp_dic[spkr].items():
                    ...
                    this_vec = el_dic['x']
                    new_vec = np.concatenate([this_vec, prev_vec], axis=0)
        ...
        with open(os.path.join(nldrp.config.BASE_PATH, 'up2now_best_features_nl_first.json'), 'w') as fd:
    ...
    try:
        with open(os.path.join(nldrp.config.BASE_PATH, 'best_features_nl_first.json'), 'w') as fd:
            json.dump(formatted_results, fd)
        df = pd.DataFrame.from_dict(formatted_results)
        df.to_csv(os.path.join(nldrp.config.BASE_PATH, 'best_features_nl_first.csv'))
    except Exception as e:
         with open(os.path.join(nldrp.config.BASE_PATH, 'best_features_nl_first.json'), 'w') as fd:
            json.dump(formatted_results, fd)
